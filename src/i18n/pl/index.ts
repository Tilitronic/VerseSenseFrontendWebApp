export default {
  nav: {
    navigation: 'Nawigacja',
    main: 'Główna',
    about: 'O projekcie',
    feedback: 'Wyślij opinię',
  },
  lang: {
    uk: 'Українська',
    pl: 'Polski',
    be: 'Беларуская',
    en: 'English',
  },
  editor: {
    title: 'Edytor poezji',
    rows: 'Wiersze',
    rowsShow: 'Pokaż ustawienia wierszy',
    rowsHide: 'Ukryj ustawienia wierszy',
    activeLineOnly: 'Tylko aktywna linia',
    everyLine: 'Wszystkie linie',
    words: 'słów',
    clear: 'Wyczyść',
    copyAll: 'Kopiuj cały tekst',
    spellcheck: 'Pisown.',
    spellcheckOn: 'Sprawdzanie pisowni przeglądarki włączone',
    spellcheckOff: 'Sprawdzanie pisowni przeglądarki wyłączone',
    spellcheckHint:
      'Ukrywa czerwone podkreślenia przeglądarki. Rozszerzenie LanguageTool zawsze widzi to pole niezależnie od tego przełącznika.',
    lt: 'LT',
    ltOn: 'LanguageTool API włączone',
    ltOff: 'LanguageTool API wyłączone',
    ltDesc:
      'Sprawdzanie gramatyki i stylu przez publiczne API LanguageTool. Rozszerzenie przeglądarki jest dokładniejsze i bez limitu zapytań.',
    ltRateInfo: 'do 20 zapytań/min · 20 KB/zapytanie',
    ltExtension: 'Rozszerzenie LanguageTool do przeglądarki',
    stressDb: 'Akcent DB',
    stressDbOn: 'Słownik włączony',
    stressDbOff: 'Słownik wyłączony',
    stressMl: 'Akcent ML',
    stressMlOn: 'Prediktor ML włączony',
    stressMlOff: 'Prediktor ML wyłączony',
  },
  stressInfo: {
    dbTitle: 'Słownik akcentów (DB)',
    dbDesc:
      'Wyszukiwanie w ua-word-stress-wasm — 3 mln form wyrazowych osadzonych w binarnym pliku WASM. Zawiera transkrypcję IPA i podział na sylaby. Homografy i warianty akcentowe zaznaczone na żółto.',
    dbLink: 'ua-stress-engine na GitHub',
    mlTitle: 'Prediktor ML — Luscinia',
    mlDesc:
      'Przewiduje akcent dla słów spoza słownika. Wyniki zaznaczone na niebiesko wymagają potwierdzenia.',
    mlWarning: '⚠ Pierwsze użycie pobiera model (~30 MB).',
    mlLink: 'ua-stress-engine na GitHub',
  },
  phonetic: {
    title: 'Wizualizacja wzorców',
    soundsWeb: 'Dźwięki',
    soundsWebTitle: 'Sieć skupień dźwiękowych',
    rhymes: 'Rymy',
    highlight: 'Podświetlenie',
    web: 'Sieć',
    alignRight: 'Do prawej',
    alignRightTitle: 'Wyrównaj wiersze do prawej (ignoruj wcięcia)',
    rhymesHighlight: 'Rymy',
    rhymesHighlightTitle: 'Podświetl powtarzające się wzorce fonemowe (rymy)',
    rhymeWeb: 'Sieć rymów',
    rhymeWebTitle: 'Połącz wystąpienia grup rymów łukami',
    rhymeFilterTitle: 'Ukrywaj krótkie rymy według minimalnej długości kody (w dźwiękach)',
    rhymeThresholdTitle: 'Minimalny próg podobieństwa do grupowania rymów',
    rhymeFilterSounds: 'dźwięki',
    rhymeFilterVowels: 'samogłoski',
    globalMetricOverlay: 'Metryka globalna',
    globalMetricOverlayTitle: 'Pokaż kompaktowy overlay metryki globalnej nad wizualizatorem',
    rowNum: 'Wiersz №',
    rowNumTitle: 'Pokaż / ukryj kolumnę numerów wierszy',
    syllables: 'Sylaby',
    syllablesTitle: 'Pokaż / ukryj kolumnę liczby sylab',
    cvRatio: 'Stosunek S:G',
    cvRatioTitle: 'Pokaż / ukryj kolumnę stosunku spółgłoska:samogłoska',
    soundsColor: 'Kolory dźwięków',
    soundsColorTitle: 'Pokaż / ukryj kolorowe podświetlenia wzorców dźwiękowych',
    rhymesPanel: 'Panel rymów',
    rhymesPanelTitle: 'Otwórz panel grup rymów',
    legend: 'Legenda',
    legendTitle: 'Legenda kolorów (otwórz w nowej karcie)',
    exportSvg: 'Eksport SVG',
    tabPhonetic: 'Fonetyka',
    servicesLoading: 'Ładowanie usług fonetycznych…',
  },
  mol: {
    noData: 'Brak danych Mol* — uruchom analizę',
    viewer3d: 'Struktura 3D pseudobiałka',
    interpretation: 'Interpretacja biofizyczna',
    structure: 'Łańcuch aminokwasowy i struktura drugorzędowa',
    lines: 'Wiersze',
    contacts: 'Mapa kontaktów',
  },
  metrics: {
    button: 'Metryki',
    title: 'Analiza wzorców',
    noData: 'Uruchom analizę, aby zobaczyć metryki',
    schema: 'Schemat odpowiedzi',
    interdependency: 'Model współzależności',
    globalScore: 'Ogólna strukturalność',
    planes: 'Płaszczyzny strukturalne',
    planeRhythm: 'Rytm',
    planeLocalPhonemePatterning: 'Lokalna patternizacja fonemiczna',
    planeSoundSequencePatterning: 'Sekwencja dźwięków / rymy',
    planePausePatterning: 'Pauzy',
    planeCrossLevelCoupling: 'Sprzężenie między poziomami',
    weights: 'Wagi płaszczyzn',
    glossary: 'Słownik metryk',
    interpretation: 'Interpretacja',
    source: 'Źródło',
    raw: 'Sygnał',
    baseline: 'Linia bazowa',
    score: 'Ocena',
    model: 'Model',
    verdictHigh: 'Wysoka gęstość wzorców dźwiękowych',
    verdictMid: 'Umiarkowana gęstość wzorców dźwiękowych',
    verdictLow: 'Niska gęstość wzorców dźwiękowych',
    verdictNone: 'Nie wykryto istotnych wzorców',
  },
  about: {
    pageTitle: 'O VerseSense',
    tagline: 'Ilościowa i jakościowa analiza oraz wizualizacja wzorców dźwiękowych w poezji',
    backBtn: 'Powrót do edytora',
    intro:
      'VerseSense to otwarte, przeglądarkowe narzędzie do głębokiej analizy ukraińskiego wiersza. Wklej utwór — silnik rozpozna akcenty, wygeneruje transkrypcję IPA, a następnie pójdzie dalej: wykryje i zwizualizuje powtarzające się wzorce dźwiękowe w całym tekście, zmierzy metrum sylabotonicze każdej linii, znajdzie grupy rymów i zbuduje mapę gęstości spółgłosek do samogłosek. Celem jest uwidocznienie dźwiękowej architektury wiersza.',
    featuresTitle: 'Funkcje',
    feat1:
      'Automatyczne rozpoznawanie akcentów — ua-word-stress-wasm, 3 mln form wyrazowych z pełną transkrypcją IPA i podziałem na sylaby; predyktor ML Luscinia dla nieznanych słów',
    feat2: 'Transkrypcja fonetyczna IPA dla każdego słowa',
    feat3: 'Wizualizacja metrum sylabotoniczne dla każdej linii',
    feat4:
      'Wykrywanie i wizualizacja wzorców dźwiękowych — powtarzające się fonemy kodowane kolorem z wagą według gęstości w całym tekście',
    feat5: 'Wykrywanie rymów i asonansów z podświetlaniem schematów ABAB / ABBA',
    feat6:
      'Stosunek spółgłoska:samogłoska (S:G) dla każdej linii — ilościowa miara gęstości dźwiękowej',
    feat7: 'Eksport SVG wizualizacji fonetycznych',
    feat8: 'W pełni offline — bez serwera, wszystkie obliczenia w przeglądarce',
    techTitle: 'Technologie',
    stressTitle: 'ua-stress-engine — odrębny projekt open-source',
    stressText:
      'Rozpoznawanie akcentów jest wyodrębnione do osobnego projektu open-source. Składa się z dwóch pakietów: ua-word-stress-wasm — plik binarny WASM z 3 mln formami wyrazowymi, pełną transkrypcją IPA, podziałem na sylaby i morfologią (bez zewnętrznych plików danych); ua-stress-ml — prediktor ML {mlModel} dla słów spoza słownika, w tym neologizmów (działa przez ONNX Runtime Web w Web Worker).',
    stressLink: 'ua-stress-engine na GitHub',
    demoTitle: 'To jest prototyp',
    demoText:
      'Analiza i wizualizacja wzorców dźwiękowych to demonstracyjny prototyp. Wyniki nie zostały zweryfikowane przez językoznawców ani filologów. Część aspektów jest celowo uproszczona lub jeszcze niezaimplementowana: algorytm podziału na sylaby, grupowanie i klasyfikacja dźwięków, wektoryzacja podobieństwa fonetycznego oraz kompatybilność fonetyczna między językami. Wszelkie zastosowania naukowe lub dydaktyczne wymagają niezależnej weryfikacji.',
    sourceTitle: 'Otwarte oprogramowanie',
    sourceText:
      'VerseSense jest wolnym i otwartym oprogramowaniem, opublikowanym na licencji AGPL-3.0.',
    sourceLink: 'VerseSense na GitHub',
  },
  contextMenu: {
    hiddenCount: '{n} ukrytych',
    showHidden: 'Pokaż',
    noHidden: 'Brak ukrytych',
    groupEdit: 'Edycja',
    copy: 'Kopiuj',
    cut: 'Wytnij',
    paste: 'Wklej',
    titleCapitalize: 'Pierwsza wielka + reszta małe',
    titleLowercase: 'Wszystkie małe',
    titleUppercase: 'Wszystkie wielkie',
    hideBtn: 'Ukryj',
    bgModeTitle: 'Tryb tła: karta otwiera się, edytor pozostaje aktywny',
    focusModeTitle: 'Tryb przejścia: przeglądarka przełącza się do nowej karty',
    bgModeLabel: 'Tło',
    focusModeLabel: 'Otwórz',
    close: 'Zamknij',
    groupDictUA: '🇺🇦 Słowniki',
    groupTranslate: '🌐 Tłumaczenie',
    groupWeb: '🔍 Internet',
  },
  stressTooltip: {
    mlPrediction: 'Predykcja ML · {model}',
    mlOverride: 'Kliknij, aby zmienić predykcję ML',
    freeVariants: 'Wolne warianty',
    contextDependent: 'Akcent kontekstowy',
    variantsHint: '{kind}: {variants} · kliknij samogłoskę, aby wybrać',
    multipleVariants: 'Wiele wariantów akcentu · kliknij samogłoskę, aby wybrać',
  },
  legend: {
    demoNotice:
      'To jest wersja demonstracyjna i może zawierać nieścisłości w transkrypcji, sylabie i analizie wzorców. Wyników nie należy używać jako źródeł naukowych.',
  },
};
