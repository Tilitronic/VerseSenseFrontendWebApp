export default {
  nav: {
    navigation: 'Navigation',
    main: 'Main',
    about: 'About',
    feedback: 'Send feedback',
  },
  lang: {
    uk: 'Українська',
    pl: 'Polski',
    be: 'Беларуская',
    en: 'English',
  },
  editor: {
    title: 'Poetry Editor',
    rows: 'Rows',
    rowsShow: 'Show row settings',
    rowsHide: 'Hide row settings',
    activeLineOnly: 'Active line only',
    everyLine: 'Every line',
    words: 'words',
    clear: 'Clear',
    copyAll: 'Copy all text',
    spellcheck: 'Spell',
    spellcheckOn: 'Browser spellcheck enabled',
    spellcheckOff: 'Browser spellcheck disabled',
    spellcheckHint:
      'Hides browser spell-check underlines. The LanguageTool extension always detects this field regardless of this toggle.',
    lt: 'LT',
    ltOn: 'LanguageTool API enabled',
    ltOff: 'LanguageTool API disabled',
    ltDesc:
      'Online grammar and style check via the LanguageTool public API. The browser extension is more accurate with no request limits.',
    ltRateInfo: 'up to 20 requests/min · 20 KB/request',
    ltExtension: 'LanguageTool browser extension',
    stressDb: 'Stress DB',
    stressDbOn: 'Dictionary enabled',
    stressDbOff: 'Dictionary disabled',
    stressMl: 'Stress ML',
    stressMlOn: 'ML predictor enabled',
    stressMlOff: 'ML predictor disabled',
  },
  stressInfo: {
    dbTitle: 'Stress Dictionary (DB)',
    dbDesc:
      'Lookup via ua-word-stress-wasm — 3 M word forms embedded in WASM. Includes IPA transcription and syllabification. Heteronyms and free stress variants highlighted in yellow.',
    dbLink: 'ua-stress-engine on GitHub',
    mlTitle: 'ML Predictor — Luscinia',
    mlDesc:
      'Predicts stress for words not found in the dictionary. Results highlighted in blue and require confirmation.',
    mlWarning: '⚠ First use downloads the model (~30 MB).',
    mlLink: 'ua-stress-engine on GitHub',
  },
  phonetic: {
    title: 'Patterns Visualization',
    soundsWeb: 'Sounds',
    soundsWebTitle: 'Sound clustering web',
    rhymes: 'Rhymes',
    highlight: 'Highlight',
    web: 'Web',
    alignRight: 'Right',
    alignRightTitle: 'Right-align rows (ignore leading tabs)',
    rhymesHighlight: 'Highlight rhymes',
    rhymesHighlightTitle: 'Highlight recurring phoneme patterns (rhymes)',
    rhymeWeb: 'Rimes',
    rhymeWebTitle: 'Connect rhyme group occurrences with arcs',
    rhymeFilterTitle: 'Hide short rhymes by minimum coda length (in sounds)',
    rhymeThresholdTitle: 'Set minimum similarity threshold for rhyme grouping',
    rhymeFilterSounds: 'sounds',
    rhymeFilterVowels: 'vowels',
    globalMetricOverlay: 'Global metric',
    globalMetricOverlayTitle: 'Show compact global metric overlay over the visualizer',
    rowNum: 'Row №',
    rowNumTitle: 'Show / hide row number column',
    syllables: 'Syllables',
    syllablesTitle: 'Show / hide syllable count column',
    cvRatio: 'C:V ratio',
    cvRatioTitle: 'Show / hide C:V ratio column',
    soundsColor: 'Sounds highlight',
    soundsColorTitle: 'Show / hide sound-pattern colour highlights',
    rhymesPanel: 'Rhymes panel',
    rhymesPanelTitle: 'Open rhyme groups panel',
    legend: 'Legend',
    legendTitle: 'Colour legend (open in new tab)',
    exportSvg: 'Export SVG',
    tabPhonetic: 'Phonetic',
    servicesLoading: 'Loading phonetic services…',
  },
  mol: {
    noData: 'No Mol* data — run analysis first',
    viewer3d: '3D pseudo-protein structure',
    interpretation: 'Biophysical interpretation',
    structure: 'Amino-acid chain & secondary structure',
    lines: 'Lines',
    contacts: 'Contact map',
  },
  metrics: {
    button: 'Metrics',
    title: 'Pattern Analysis',
    noData: 'Run analysis to view metrics',
    schema: 'Response schema',
    interdependency: 'Interdependency model',
    globalScore: 'Overall Structurality',
    planes: 'Structural Planes',
    planeRhythm: 'Rhythm',
    planeLocalPhonemePatterning: 'Local phoneme patterning',
    planeSoundSequencePatterning: 'Sound sequence / rhyme',
    planePausePatterning: 'Pause patterning',
    planeCrossLevelCoupling: 'Cross-level coupling',
    weights: 'Plane Weights',
    glossary: 'Metric glossary',
    interpretation: 'Interpretation',
    source: 'Source',
    raw: 'Signal',
    baseline: 'Baseline',
    score: 'Score',
    model: 'Model',
    verdictHigh: 'High density of sound patterns',
    verdictMid: 'Moderate density of sound patterns',
    verdictLow: 'Low density of sound patterns',
    verdictNone: 'No significant patterns detected',
  },
  about: {
    pageTitle: 'About VerseSense',
    tagline: 'Quantitative and qualitative analysis and visualisation of sound patterns in poetry',
    backBtn: 'Back to Editor',
    intro:
      'VerseSense is a browser-based open-source tool for deep analysis of Ukrainian verse. Paste a poem — the engine resolves word stress, generates IPA transcription, and then goes further: it detects and visualises recurring sound patterns across the whole text, measures syllabic metre per line, finds rhyme groups, and gives you a C:V density map. The goal is to make the sonic architecture of a poem visible.',
    featuresTitle: 'Features',
    feat1:
      'Automatic word-stress resolution — ua-word-stress-wasm, 3 M word forms with full IPA transcription and syllabification; Luscinia ML fallback for unknown words',
    feat2: 'IPA phonetic transcription for each word',
    feat3: 'Syllabic-accentual metre visualisation per line',
    feat4:
      'Sound-pattern detection and visualisation — recurring phonemes are colour-coded and weighted by density across the whole text',
    feat5: 'Rhyme and assonance detection with ABAB / ABBA scheme highlighting',
    feat6: 'C:V (consonant-to-vowel) ratio per line — quantitative sound-density metric',
    feat7: 'SVG export of phonetic visualisations',
    feat8: 'Fully offline — no server, all computation runs in the browser',
    techTitle: 'Built with',
    stressTitle: 'ua-stress-engine — a standalone open-source project',
    stressText:
      'Word-stress resolution lives in a separate open-source project. It includes two packages: ua-word-stress-wasm — a WASM binary with 3 M word forms, full IPA transcription, syllabification, and morphology (no data files to serve); ua-stress-ml — the {mlModel} ML predictor for words outside the dictionary, including neologisms (runs via ONNX Runtime Web in a Web Worker).',
    stressLink: 'ua-stress-engine on GitHub',
    demoTitle: 'This is a prototype',
    demoText:
      'The sound-pattern analysis and visualisation features are a proof-of-concept demo. Results have not been reviewed or validated by linguists or philologists. Several aspects are intentionally simplified or not yet implemented: the syllabification algorithm, phoneme grouping and classification, phonetic similarity vectorisation, and cross-language phonetic compatibility. Any use in academic or educational contexts requires independent verification.',
    sourceTitle: 'Open source',
    sourceText:
      'VerseSense is free and open-source software, published under the AGPL-3.0 licence.',
    sourceLink: 'VerseSense on GitHub',
  },
  contextMenu: {
    hiddenCount: '{n} hidden',
    showHidden: 'Show',
    noHidden: 'No hidden items',
    groupEdit: 'Edit',
    copy: 'Copy',
    cut: 'Cut',
    paste: 'Paste',
    titleCapitalize: 'First uppercase + rest lowercase',
    titleLowercase: 'All lowercase',
    titleUppercase: 'All uppercase',
    hideBtn: 'Hide',
    bgModeTitle: 'Background mode: tab opens, editor stays active',
    focusModeTitle: 'Focus mode: browser switches to new tab',
    bgModeLabel: 'Bg',
    focusModeLabel: 'Open',
    close: 'Close',
    groupDictUA: '🇺🇦 Dictionaries',
    groupTranslate: '🌐 Translate',
    groupWeb: '🔍 Web',
  },
  stressTooltip: {
    mlPrediction: 'ML prediction · {model}',
    mlOverride: 'Click to override ML prediction',
    freeVariants: 'Free variants',
    contextDependent: 'Context-dependent stress',
    variantsHint: '{kind}: {variants} · click a vowel to choose',
    multipleVariants: 'Multiple stress variants · click a vowel to choose',
  },
  legend: {
    demoNotice:
      'This is a demo version and may contain inaccuracies in transcription, syllabification, and pattern analysis. Results should not be used as academic sources.',
  },
};
